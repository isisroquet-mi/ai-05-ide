...
- `LowTypeWageAdvantageSpec`: algebraic wage comparison in the two-type
  discrete reduction used to explain Proposition 6.
- `StrictOutputGainBridgeSpec`: the missing strictness step in the hand
  derivation of the output comparison.
-/

namespace IT25KnowledgeEconomy

/-- In the two-type reduction, the zero-profit wage formulas imply that the
low type earns strictly more with non-autonomous AI.  The formulas themselves
remain explicit premises rather than being silently attributed to the full
...
/-- A proper autonomous-only opportunity with positive output is the extra
premise needed to strengthen feasible-set monotonicity from `≥` to `>`.
This is a bridge for Proposition 6's aggregate-output clause, not a
formalization of the full equilibrium construction. -/
def StrictOutputGainBridgeSpec : Prop :=
  ∀ {yN yA extra : ℝ},
    0 < extra →
    yA = yN + extra →
    yA > yN
