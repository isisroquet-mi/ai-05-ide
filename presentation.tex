...
\setbeamertemplate{footline}{\begin{beamercolorbox}[wd=\paperwidth,ht=2.7ex,dp=1.1ex,leftskip=.55cm,rightskip=.55cm]{author in head/foot}\color{Gris}\scriptsize Ide y Talam\`as (2025) --- IA en la econom\'ia del conocimiento\hfill\textcolor{Petroleo}{\insertframenumber/\inserttotalframenumber}\end{beamercolorbox}}
...
\column{0.53\textwidth}
\centering
\IfFileExists{hand/derivacion_manual_proposicion_6.pdf}{\fcolorbox{Turquesa}{white}{\includegraphics[page=1,width=0.96\linewidth,height=0.67\textheight,trim=20 580 20 22,clip,keepaspectratio]{hand/derivacion_manual_proposicion_6.pdf}}}{\fcolorbox{Turquesa}{white}{\parbox[c][0.60\textheight][c]{0.92\linewidth}{\centering\color{Gris}Inserte aqu\'i la derivaci\'on manual.}}}
...
\end{block}
\column{0.48\textwidth}
\clave{Statement Lean}
\vspace{0.08cm}
{\scriptsize\ttfamily
def LowTypeWageAdvantageSpec : Prop :=\\
\quad forall \{a n : Real\},\\
\qquad 0 < a -> 1 < n ->\\
\qquad a > a * (1 - 1 / n)\par}
\vspace{0.25cm}
{\small Los reales representan salarios, capacidad y span of control. Las hip\'otesis excluyen IA sin capacidad y equipos degenerados.}
\end{columns}
\end{frame}

\begin{frame}{Lean: fragmento de prueba y alcance}
\begin{columns}[T,onlytextwidth]
\column{0.58\textwidth}
\clave{Paso relevante}
\vspace{0.08cm}
{\scriptsize\ttfamily
have hn0 : 0 < n := lt\_trans zero\_lt\_one hn\\
have hgap : 0 < a / n := div\_pos ha hn0\\
have hid : a - a * (1 - 1 / n) = a / n := by\\
\quad field\_simp\\
\quad ring\\
linarith [hgap, hid]\par}
\vspace{0.22cm}
{\footnotesize La prueba transforma la diferencia salarial en $a/n$, demuestra que el denominador es positivo y cierra el orden con aritm\'etica lineal.}
\column{0.38\textwidth}
\clave{Qu\'e verifica}
\begin{itemize}\footnotesize
\item Los supuestos visibles bastan.
\item No hay divisi\'on por cero escondida.
\item La conclusi\'on coincide con el mecanismo discreto.
\end{itemize}
\clave{Qu\'e no verifica}
\begin{itemize}\footnotesize
\item El equilibrio continuo.
\item Los umbrales $\bar z_{AI}$ o $h_0$.
\item La Proposici\'on 6 completa.
\end{itemize}
{\scriptsize\color{Gris}El estado exacto se reporta dentro de \texttt{lean/}.}
\end{columns}
\end{frame}
